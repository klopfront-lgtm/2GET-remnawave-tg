...
class Settings(BaseSettings):
    ...
    # Redis Configuration
    REDIS_ENABLED: bool = Field(default=False, description="Enable Redis for FSM storage and caching")
    REDIS_HOST: str = Field(default="localhost")
    REDIS_PORT: int = Field(default=6379)
    REDIS_PASSWORD: Optional[str] = Field(default=None)
    REDIS_FSM_DB: int = Field(default=0, description="Redis database for FSM storage")
    REDIS_CACHE_DB: int = Field(default=1, description="Redis database for caching")
    REDIS_FSM_STATE_TTL: int = Field(default=3600, description="FSM state TTL in seconds")
    REDIS_FSM_DATA_TTL: int = Field(default=3600, description="FSM data TTL in seconds")
    
    # Rate Limiting Configuration
    RATE_LIMIT_ENABLED: bool = Field(default=True, description="Enable rate limiting middleware")
    RATE_LIMIT_MAX_REQUESTS: int = Field(default=20, description="Maximum requests per time window")
    RATE_LIMIT_TIME_WINDOW: int = Field(default=60, description="Time window in seconds")
    RATE_LIMIT_BAN_DURATION: int = Field(default=300, description="Temporary ban duration in seconds (0 = no ban)")
    RATE_LIMIT_ADMIN_EXEMPT: bool = Field(default=True, description="Exempt admins from rate limiting")
...
def get_settings() -> Settings:
    ...
    if _settings_instance is None:
        try:
            ...
            # Маскируем чувствительные поля в ValidationError перед логированием
            sensitive_keywords = ['password', 'key', 'token', 'secret', 'api']
            
            # Получаем строковое представление ошибки
            error_str = str(e)
            
            # Маскируем чувствительные данные в сообщении об ошибке
            for error in e.errors():
                field_name = str(error.get('loc', [''])[0]).lower()
                if any(keyword in field_name for keyword in sensitive_keywords):
                    # Заменяем значение на замаскированное
                    if 'input' in error:
                        error['input'] = '***MASKED***'
                    if 'ctx' in error and isinstance(error['ctx'], dict):
                        for ctx_key in error['ctx']:
                            if isinstance(error['ctx'][ctx_key], str):
                                error['ctx'][ctx_key] = '***MASKED***'
            
            # Создаем безопасное сообщение об ошибке
            safe_error_msg = "Validation error in sensitive fields. Check .env configuration."
            error_details = []
            for error in e.errors():
                field_name = '.'.join(str(loc) for loc in error.get('loc', []))
                error_type = error.get('type', 'unknown')
                # Не включаем значения в сообщение
                error_details.append(f"Field '{field_name}': {error_type}")
            
            safe_error_msg += " Fields with errors: " + ", ".join(error_details)
            
            logging.critical(
                f"Pydantic validation error while loading settings: {safe_error_msg}")

            raise SystemExit(
                f"CRITICAL SETTINGS ERROR: {safe_error_msg}. Please check your .env file and Settings model."
            )
