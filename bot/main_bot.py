...
# PERFORMANCE: MemoryStorage is used for FSM state management
# WARNING: MemoryStorage stores all state in RAM and will be lost on bot restart
# For production environments with multiple bot instances or high availability requirements,
# consider migrating to RedisStorage for persistent state management
#
# Example RedisStorage configuration:
...
#
# Benefits of RedisStorage:
# - Persistent state across bot restarts
# - Shared state across multiple bot instances (horizontal scaling)
# - Automatic TTL (Time To Live) for expired states
# - Better memory management for high-load scenarios
#
# TODO: Implement RedisStorage for production deployment
# TODO: Add FSM state cleanup task if staying with MemoryStorage
...
from bot.utils.graceful_shutdown import get_shutdown_manager, SignalHandler
...
async def on_startup_configured(dispatcher: Dispatcher):
    ...
    if telegram_webhook_url_to_set:
        # SECURITY FIX: Use hash of BOT_TOKEN instead of token itself in webhook URL
        # This prevents token exposure in logs and external monitoring systems
        token_hash = hashlib.sha256(settings.BOT_TOKEN.encode()).hexdigest()[:16]
        full_telegram_webhook_url = (
            f"{str(telegram_webhook_url_to_set).rstrip('/')}/webhook/{token_hash}"
        )

        logging.info(
            f"STARTUP: Attempting to set Telegram webhook to: /webhook/[hash] (token hash used for security)"
        )
    ...
    # REFACTOR: Run panel sync in background to avoid blocking bot startup
    async def background_panel_sync():
        """Background task for panel synchronization on startup."""
        try:
            logging.info("STARTUP: Starting automatic panel sync in background...")
            ...
            if sync_result.get("status") == "completed":
                logging.info(f"STARTUP: Background sync completed successfully. Details: {sync_result.get('details', 'N/A')}")
            else:
                logging.warning(f"STARTUP: Background sync completed with issues. Status: {sync_result.get('status', 'unknown')}")
                
        except Exception as e:
            logging.error(f"STARTUP: Background sync failed: {e}", exc_info=True)
    
    # Create background task for panel sync
    asyncio.create_task(background_panel_sync(), name="PanelSyncTask")
    logging.info("STARTUP: Panel sync task created and running in background")
...
async def on_shutdown_configured(dispatcher: Dispatcher):
    ...
    try:
        ...
        if RedisStorageFactory.is_connected():
            await RedisStorageFactory.close()
            logging.info("SHUTDOWN: Redis FSM storage closed")
    except Exception as e:
        logging.warning(f"SHUTDOWN: Error closing Redis storage: {e}")
...
async def run_bot(settings_param: Settings):
    ...
    # Create FSM storage (MemoryStorage or RedisStorage based on config)
    from bot.app.controllers.dispatcher_controller import create_storage
    storage = await create_storage(settings_param)
    
    # Build dispatcher with storage
    dp, bot, extra = build_dispatcher(settings_param, local_async_session_factory)
    # Replace temporary storage with actual one
    dp.storage = storage
    ...
    signal_handler = SignalHandler(shutdown_manager)
    signal_handler.setup_signal_handlers()
    
    # Register shutdown handler
    async def cleanup_on_shutdown():
        """Cleanup resources on shutdown."""
        logging.info("Graceful shutdown: Starting cleanup process...")
        
        # Cancel all running tasks
        for task in main_tasks:
            if task and not task.done():
                logging.info(f"Graceful shutdown: Cancelling task '{task.get_name()}'")
                task.cancel()
                try:
                    await task
                except asyncio.CancelledError:
                    logging.info(f"Graceful shutdown: Task '{task.get_name()}' cancelled successfully")
                except Exception as e:
                    logging.error(f"Graceful shutdown: Error cancelling task '{task.get_name()}': {e}")
        
        # Cleanup web app runner
        if web_app_runner:
            try:
                await web_app_runner.cleanup()
                logging.info("Graceful shutdown: AIOHTTP AppRunner cleaned up")
            except Exception as e:
                logging.error(f"Graceful shutdown: Error cleaning up AppRunner: {e}")
        
        # Emit dispatcher shutdown
        try:
            await dp.emit_shutdown()
            logging.info("Graceful shutdown: Dispatcher shutdown emitted")
        except Exception as e:
            logging.error(f"Graceful shutdown: Error emitting shutdown: {e}")
    
    shutdown_manager.register_shutdown_handler(cleanup_on_shutdown)
    ...
    logging.info("Graceful shutdown: Press Ctrl+C to stop the bot gracefully")

    try:
        # Wait for shutdown signal instead of gathering tasks
        await shutdown_manager.wait_for_shutdown()
        logging.info("Graceful shutdown: Shutdown signal received")
    except (KeyboardInterrupt, SystemExit) as e:
        logging.info(f"Graceful shutdown: Received interrupt: {type(e).__name__}")
        await shutdown_manager.initiate_shutdown("KeyboardInterrupt/SystemExit")
    except Exception as e:
        logging.error(f"Graceful shutdown: Unexpected error: {e}", exc_info=True)
        await shutdown_manager.initiate_shutdown("Exception")
    finally:
