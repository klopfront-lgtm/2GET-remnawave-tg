...
async def build_and_start_web_app(
    ...
    if telegram_uses_webhook_mode:
        # SECURITY FIX: Use hash of BOT_TOKEN instead of token itself in URL
        # This prevents token leakage in logs, monitoring tools, and reverse proxy logs
        token_hash = hashlib.sha256(settings.BOT_TOKEN.encode()).hexdigest()[:16]
        telegram_webhook_path = f"/webhook/{token_hash}"
        app.router.add_post(telegram_webhook_path, SimpleRequestHandler(dispatcher=dp, bot=bot))
        logging.info(
            f"Telegram webhook route configured at: [POST] /webhook/[hash] (relative to base URL)"
        )
