...
class FreeKassaService:
    ...
    async def webhook_route(self, request: web.Request) -> web.Response:
        ...
        async with self.async_session_factory() as session:
            ...
            # Определяем тип платежа из описания или других данных
            payment_type = "subscription"  # По умолчанию подписка
            if payment.description and "баланс" in payment.description.lower():
            ...
            try:
                ...
                if payment_type == "balance":
                    ...
                    new_balance = await balance_service.get_balance(session, payment.user_id)
                    text = _(
                        "balance_deposit_success",
                        default=f"✅ Баланс успешно пополнен на {payment.amount} {self.default_currency}!\n\n"
                               f"Текущий баланс: {new_balance} {self.default_currency}"
                    )
                    
                    try:
                        await self.bot.send_message(payment.user_id, text, parse_mode="HTML")
                    except Exception as e:
                        logging.error(f"FreeKassa: failed to send balance deposit message to user {payment.user_id}: {e}")
                    
                    logging.info(f"FreeKassa balance deposit successful for user {payment.user_id}: +{payment.amount}")
                    return web.Response(text="YES")
