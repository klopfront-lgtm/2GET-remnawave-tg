...
class SubscriptionService:
    """
    Main service for managing VPN subscriptions.
    
    Handles:
    - Trial subscription activation
    - Paid subscription activation and renewal
    - Subscription extension and management
    - Panel user creation and synchronization
    - Auto-renewal processing
    
    REFACTOR: Core service with extracted helper classes for better organization.
    """
    ...
    def __init__(
        ...
        # REFACTOR: Initialize helper classes for better code organization
    ...
    async def activate_trial_subscription(
        ...
        Returns:
            Dict with activation details or error info
        """
    ...
    async def activate_subscription(
        ...
        Args:
            ...
            months: Количество месяцев подписки (используется если tariff_id не указан)
            payment_amount: Сумма оплаты
            payment_db_id: ID платежа
            promo_code_id_from_payment: ID промокода из платежа
            provider: Провайдер платежа
            tariff_id: ID тарифа (опционально, для новой системы тарифов)
            keep_other_active: Сохранять ли другие активные подписки (по умолчанию True)
            subscription_name: Название подписки (генерируется автоматически если не указано)
        ...
        # Проверить лимит подписок ПЕРЕД активацией
        can_create, current_count, max_limit = await subscription_dal.check_subscription_limit(
            session, user_id
        )

        if not can_create:
            logging.warning(
                f"User {user_id} exceeded subscription limit: {current_count}/{max_limit}"
            )
            raise ValueError(
                f"Exceeded subscription limit. Current: {current_count}, Max: {max_limit}"
            )

        logging.info(
            f"Subscription limit check passed for user {user_id}: {current_count}/{max_limit}"
        )

        # Получаем тариф, если указан tariff_id
        tariff: Optional[Tariff] = None
        if tariff_id:
            tariff = await tariff_dal.get_tariff_by_id(session, tariff_id)
            if not tariff:
                logging.error(f"Tariff {tariff_id} not found for subscription activation")
                return None
            if not tariff.is_active:
                logging.error(f"Tariff {tariff_id} is not active")
                return None
            logging.info(f"Using tariff {tariff_id} ({tariff.name}) for user {user_id}")
        ...
        # Определяем продолжительность на основе тарифа или месяцев
        if tariff:
            # Используем duration_days из тарифа
            duration_days_total = tariff.duration_days
            end_after_tariff = start_date + timedelta(days=duration_days_total)
            logging.info(
                f"Using tariff duration: {duration_days_total} days "
                f"(from {start_date} to {end_after_tariff})"
            )
        else:
            # Старая логика: base duration by months
            end_after_months = add_months(start_date, months)
        ...
        # Деактивируем другие подписки только если явно указано
        if not keep_other_active:
            await subscription_dal.deactivate_other_active_subscriptions(
                session, panel_user_uuid, panel_sub_link_id
            )
            logging.info(f"Deactivated other subscriptions for user {user_id}")
        else:
            logging.info(f"Keeping other subscriptions active for user {user_id}")
        ...
        # Определяем traffic_limit_bytes из тарифа или дефолтных настроек
        traffic_limit_bytes = (
        ...
        # Определяем device_limit из тарифа (если есть, будет передан в панель)
        device_limit = tariff.device_limit if tariff else None

        # Генерировать название если не указано
        if not subscription_name:
            active_count = await subscription_dal.count_active_subscriptions(session, user_id)
            if active_count == 0:
                subscription_name = "Основная подписка"
            else:
                subscription_name = f"Подписка #{active_count + 1}"
        
        # Проверить, есть ли уже главная подписка
        primary_sub = await subscription_dal.get_primary_subscription(session, user_id)
        is_primary = False
        if not primary_sub:
            # Это первая подписка - сделать главной
            is_primary = True
            logging.info(f"Setting subscription as primary for user {user_id}")
        
        sub_payload = {
            "user_id": user_id,
            "tariff_id": tariff_id,  # Добавляем tariff_id в подписку
            "panel_user_uuid": panel_user_uuid,
            "panel_subscription_uuid": panel_sub_link_id,
            "start_date": start_date,
            "end_date": final_end_date,
            "duration_months": months if not tariff else 0,  # 0 если используется тариф
            "is_active": True,
        ...
        logging.info(
            f"Creating subscription with traffic_limit={traffic_limit_bytes} bytes, "
            f"device_limit={device_limit}, tariff_id={tariff_id}"
        )
        try:
        ...
        # Обновляем панель с учетом лимитов из тарифа
        panel_update_payload = self._build_panel_update_payload(
            panel_user_uuid=panel_user_uuid,
            expire_at=final_end_date,
            status="ACTIVE",
            traffic_limit_bytes=traffic_limit_bytes,
            device_limit=device_limit,  # Передаем device_limit из тарифа
        )
    ...
    async def extend_active_subscription_days(
        ...
        """
        # REFACTOR: Use helper to determine traffic limit
        apply_main_traffic_limit = self.activation_helper.should_apply_main_traffic_limit(reason)
    ...
    async def charge_subscription_renewal(
        ...
        # Создаем платеж через YooKassa с сохраненным методом оплаты
    ...
    async def delete_subscription(
        ...
        """
        Удаление подписки без возврата средств.
        Если купил - значит купил.
        
        Args:
            session: DB session
            subscription_id: ID подписки
            user_id: ID пользователя (для проверки прав)
            admin_override: Админский override (игнорирует can_be_deleted)
        ...
        if not subscription.is_active:
            return False, "subscription_already_inactive"
        
        if not subscription.can_be_deleted and not admin_override:
            return False, "subscription_cannot_be_deleted"
        
        # Деактивировать подписку
        subscription.is_active = False
        
        # Если это была главная - переназначить
        if subscription.is_primary:
            # Найти другую активную подписку
            other_subs = await subscription_dal.get_active_subscriptions_for_user(session, user_id)
            if other_subs:
                # Первую сделать главной
                await subscription_dal.set_primary_subscription(
                    session, other_subs[0].subscription_id, user_id
                )
        
        # Обновить на панели (отключить)
        try:
            await self.panel_service.update_user_details_on_panel(
            ...
            logging.error(f"Failed to disable subscription on panel: {e}")
        
        await session.flush()
        
        logging.info(f"Subscription {subscription_id} deleted for user {user_id}")
        return True, "subscription_deleted"
    ...
    async def get_all_user_subscriptions_with_details(
        ...
        """
        Получить все активные подписки пользователя с детальной информацией.
        
        Returns:
            List[Dict] с полями: subscription_id, name, tariff, end_date, 
                                 traffic_limit, traffic_used, device_limit, is_primary
        """
        subscriptions = await subscription_dal.get_active_subscriptions_for_user(session, user_id)
        
        result = []
        for sub in subscriptions:
            # Получить данные с панели
            panel_data = {}
            try:
                panel_user = await self.panel_service.get_user_by_uuid(sub.panel_user_uuid)
                if panel_user:
                    panel_data = {
                        'traffic_used': panel_user.get('usedTrafficBytes', 0),
                        'status_from_panel': panel_user.get('status', 'UNKNOWN'),
                    }
            except Exception as e:
                logging.error(f"Failed to get panel data for subscription {sub.subscription_id}: {e}")
            
            result.append({
                'subscription_id': sub.subscription_id,
                'name': sub.subscription_name or f"Подписка #{sub.subscription_id}",
                'tariff_name': sub.tariff.name if sub.tariff else "Unknown",
                'start_date': sub.start_date,
                'end_date': sub.end_date,
                'traffic_limit': sub.get_effective_traffic_limit(),
                'traffic_used': panel_data.get('traffic_used', 0),
                'device_limit': sub.get_effective_device_limit(),
                'is_primary': sub.is_primary,
                'can_be_deleted': sub.can_be_deleted,
                'panel_status': panel_data.get('status_from_panel', sub.status_from_panel),
            })
