...
def _mask_tribute_payload(payload: dict) -> dict:
    """
    Mask sensitive data in Tribute webhook payload before logging.
    
    SECURITY: Removes or masks PII from logs to comply with data protection regulations.
    """
    safe_payload = payload.copy()
    data = safe_payload.get("payload", {})
    if isinstance(data, dict):
        safe_data = data.copy()
        # Mask user_id partially
        if "telegram_user_id" in safe_data:
            user_id_str = str(safe_data["telegram_user_id"])
            safe_data["telegram_user_id"] = f"***{user_id_str[-4:]}" if len(user_id_str) > 4 else "***"
        # Remove other potentially sensitive fields
        for sensitive_field in ["email", "phone", "customer_email", "customer_phone"]:
            if sensitive_field in safe_data:
                safe_data[sensitive_field] = "***MASKED***"
        safe_payload["payload"] = safe_data
    return safe_payload
...
class TributeService:
    ...
    async def handle_webhook(self, raw_body: bytes, signature_header: Optional[str]) -> web.Response:
        ...
        try:
            ...
            logging.warning(f"SECURITY: Invalid JSON in Tribute webhook: {e}")
            return bad_request("invalid_json")
        except Exception as e:
            logging.error(f"SECURITY: Unexpected error decoding Tribute webhook: {e}")
            return bad_request("invalid_json")

        # SECURITY FIX: Mask PII (user_id, emails, phones) before logging
        masked_payload = _mask_tribute_payload(payload)
        logging.info(
            "Tribute webhook data (PII masked): %s",
        ...
        # Определяем тип платежа (subscription или balance)
        ...
        logging.info(f"Tribute webhook: payment_type={payment_type}, user_id={user_id}, amount={amount_value}")
            ...
            if event_name == "new_subscription":
                ...
                if payment_type == "balance":
                    ...
                    await balance_service.deposit(
                        ...
                        user_id=int(user_id),
                        amount=amount_float,
                        description="Пополнение баланса через Tribute",
                        currency=currency
                    )
                    ...
                    new_balance = await balance_service.get_balance(session, int(user_id))
                    text = _(
                        "balance_deposit_success",
                        default=f"✅ Баланс успешно пополнен на {amount_float} {currency}!\n\n"
                               f"Текущий баланс: {new_balance} {currency}"
                    )
                    
                    try:
                        await bot.send_message(int(user_id), text, parse_mode="HTML")
                    except Exception as e:
                        logging.error(f"Failed to send Tribute balance deposit message to user {user_id}: {e}")
                    
                    logging.info(f"Tribute balance deposit successful for user {user_id}: +{amount_float}")
                    return ok({"event": event_name, "type": "balance"})
