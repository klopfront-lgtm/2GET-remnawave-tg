...
class PromoCodeService:
    ...
    async def apply_promo_code(
        ...
        code_input: str,
        user_lang: str,
    ) -> Tuple[bool, Union[datetime, str, Dict]]:
        ...
        # Обработка промокодов типа 'balance'
        if promo_data.type == 'balance':
            balance_amount = promo_data.value
            
            # Начислить баланс через BalanceService
            await self.balance_service.add_bonus(
                session=session,
                user_id=user_id,
                amount=balance_amount,
                description=f"Промокод {code_input_upper}"
            )
            ...
            logging.info(f"User {user_id} activated balance promo {code_input_upper}, amount: {balance_amount}")
            
            # Вернуть информацию о балансе
            return True, {"type": "balance", "amount": balance_amount}
        
        # Обработка промокодов типа 'bonus_days' (существующий код)
        elif promo_data.type == 'bonus_days':
            ...
            logging.error(f"Unknown promo code type: {promo_data.type} for code {code_input_upper}")
            return False, _("error_applying_promo_bonus")
