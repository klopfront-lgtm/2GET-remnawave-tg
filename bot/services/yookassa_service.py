...
def _mask_pii(data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Mask personally identifiable information in data before logging.
    
    SECURITY: This helper masks email, phone, and other PII to comply with data protection.
    """
    masked = dict(data)
    if "customer" in masked and isinstance(masked["customer"], dict):
        ...
        if "email" in customer:
            ...
            if "@" in email:
                parts = email.split("@")
                customer["email"] = f"{parts[0][:2]}***@{parts[1]}"
            else:
                customer["email"] = "***@***"
        if "phone" in customer:
            phone = customer["phone"]
            # Mask phone: show only last 4 digits
            customer["phone"] = f"***{phone[-4:]}" if len(phone) >= 4 else "***"
...
class YooKassaService:
    ...
    async def create_payment(
            ...
            payment_type: str = "subscription") -> Optional[Dict[str, Any]]:
        """
        ...
        Args:
            ...
            save_payment_method: Сохранить метод оплаты
            payment_method_id: ID ранее сохраненного метода оплаты
            capture: Автоматическое списание
            bind_only: Только привязка карты
        ...
        try:
            ...
            # Добавляем payment_type в метаданные для идентификации типа платежа
            enhanced_metadata = dict(metadata)
            enhanced_metadata["payment_type"] = payment_type
            builder.set_metadata(enhanced_metadata)
            
            logging.info(f"Creating YooKassa payment with type: {payment_type}")
            ...
            # SECURITY FIX: Mask PII (email, phone) in receipt before logging
            masked_receipt = _mask_pii(receipt_data_dict)
            logging.info(
                f"Creating YooKassa payment (Idempotence-Key: {idempotence_key}). "
                f"Amount: {amount} {currency}. Metadata: {metadata}. Receipt: {masked_receipt}"
            )
