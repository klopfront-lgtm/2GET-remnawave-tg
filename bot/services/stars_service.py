...
class StarsService:
    ...
    async def create_invoice(self, session: AsyncSession, user_id: int, months: int,
                             stars_price: int, description: str, payment_type: str = "subscription") -> Optional[int]:
        """
        ...
        Args:
            ...
            months: Количество месяцев подписки (для subscription)
            stars_price: Цена в звездах
            description: Описание платежа
            payment_type: Тип платежа ("subscription" или "balance")
            
        Returns:
            ID созданного платежа или None при ошибке
        """
        payment_record_data = {
            "user_id": user_id,
        ...
        logging.info(f"Creating Telegram Stars invoice with type: {payment_type}")
        try:
        ...
        # Добавляем payment_type в payload
        payload = f"{db_payment_record.payment_id}:{months}:{payment_type}"
    ...
    async def process_successful_payment(self, session: AsyncSession,
                                         ...
                                         stars_amount: int,
                                         i18n_data: dict,
                                         payment_type: str = "subscription") -> None:
        """
        Обработать успешный платеж через Telegram Stars.
        
        Args:
            session: Сессия БД
            message: Сообщение с успешным платежом
            payment_db_id: ID платежа в БД
            months: Количество месяцев (для subscription)
            stars_amount: Сумма в звездах
            i18n_data: Данные для интернационализации
            payment_type: Тип платежа ("subscription" или "balance")
        """
        try:
        ...
        if payment_type == "balance":
            logging.info(f"Processing balance deposit via Stars for user {message.from_user.id}, amount: {stars_amount}")
            ...
            new_balance = await balance_service.get_balance(session, message.from_user.id)
            success_msg = _(
                "balance_deposit_success",
                default=f"✅ Баланс успешно пополнен на {stars_amount} звезд!\n\n"
                       f"Текущий баланс: {new_balance} XTR"
            )
            ...
            try:
                ...
                logging.error(f"Failed to send balance deposit message: {e_send}")
            
            logging.info(f"Balance deposit via Stars successful for user {message.from_user.id}: +{stars_amount}")
            return
