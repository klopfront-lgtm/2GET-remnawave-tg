...
from bot.keyboards.inline.admin_keyboards import (
    ...
    get_tariffs_pricing_keyboard, get_tariff_management_keyboard,
...
from . import tariff_management as admin_tariff_handlers
from . import discount_management as admin_discount_handlers
...
async def admin_panel_actions_callback_handler(
    ...
    if action == "stats":
        ...
        await admin_tariff_handlers.tariff_management_handler(
            callback, i18n_data, settings, session)
    elif action == "tariffs_list" and len(action_parts) > 2:
        try:
            page = int(action_parts[2])
            await admin_tariff_handlers.tariffs_list_handler(
                callback, i18n_data, settings, session, page)
        except (IndexError, ValueError):
            await callback.answer("Invalid page number", show_alert=True)
    elif action == "create_tariff":
        await admin_tariff_handlers.create_tariff_start(
            callback, state, i18n_data, settings)
    elif action == "discount_management":
        await admin_discount_handlers.discount_management_handler(
            callback, i18n_data, settings, session)
    elif action == "set_discount":
        await admin_discount_handlers.set_discount_start(
            callback, state, i18n_data, settings)
    elif action == "view_discounts":
        await admin_discount_handlers.view_discounts_start(
            callback, state, i18n_data, settings)
    elif action == "main":
        try:
...
async def admin_section_handler(callback: types.CallbackQuery, state: FSMContext, 
    ...
    try:
        ...
        elif section == "tariffs_pricing":
            await callback.message.edit_text(
                _("admin_tariffs_pricing_section"),
                reply_markup=get_tariffs_pricing_keyboard(i18n, current_lang)
            )
        else:
            await callback.answer(_("admin_unknown_action"), show_alert=True)
            return
