...
async def _initiate_yk_payment(
    ...
    *,
    ...
    # Добавляем tariff_id если указан
    if tariff_id:
        payment_record_data["tariff_id"] = tariff_id
    ...
    if tariff_id:
        yookassa_metadata["tariff_id"] = str(tariff_id)
...
async def select_subscription_period_callback_handler(callback: types.CallbackQuery, settings: Settings, i18n_data: dict, session: AsyncSession):
    ...
    try:
        ...
        logging.warning(f"Failed to get user balance: {e}")
...
async def pay_yk_callback_handler(callback: types.CallbackQuery, settings: Settings, i18n_data: dict, yookassa_service: YooKassaService, session: AsyncSession):
    ...
    await _initiate_yk_payment(
        ...
        tariff_id=None,  # Старая система без тарифов
    )
    try:
        await callback.answer()
    except Exception:
...
async def pay_fk_callback_handler(
    ...
    # Добавляем tariff_id если передан в metadata (для новой системы тарифов)
    # tariff_id будет None для старой системы с месяцами
...
async def pay_subscription_from_balance(
    ...
    # Парсинг данных: pay_balance:months:price
    try:
        parts = callback.data.split(":")
        if len(parts) != 3:
            await callback.answer(get_text("error_invalid_callback_data"), show_alert=True)
            return
        
        months = int(parts[1])
        price = float(parts[2])
    except (ValueError, IndexError) as e:
        logging.error(f"Invalid pay_balance callback data: {callback.data}, error: {e}")
        await callback.answer(get_text("error_try_again"), show_alert=True)
        return
    ...
    try:
        ...
        if not user:
            await callback.answer(get_text("error_user_not_found"), show_alert=True)
            return
        ...
        if current_balance < price:
            await callback.answer(
                get_text("insufficient_balance_for_subscription",
        ...
        # Списать баланс
        try:
            await balance_service.record_payment(
                session=session,
                user_id=user_id,
                amount=price,
            ...
            if "InsufficientFunds" in str(e) or "insufficient" in str(e).lower():
                await callback.answer(
                    get_text("insufficient_balance_for_subscription",
                             balance=current_balance,
                             price=price,
                ...
                logging.error(f"Balance payment error: {e}", exc_info=True)
                await session.rollback()
                await callback.answer(get_text("error_payment_failed"), show_alert=True)
                return
        ...
        payment_data = {
            ...
            "status": "succeeded",
            "provider": "balance",
            "description": f"Оплата подписки на {months} мес. с баланса",
            "subscription_duration_months": months,
        }
        
        payment = await payment_dal.create_payment_record(session, payment_data)
        if not payment:
            logging.error(f"Failed to create payment record for user {user_id}")
            await session.rollback()
            await callback.answer(get_text("error_creating_payment_record"), show_alert=True)
            return
        
        logging.info(f"Balance payment {payment.payment_id} created for user {user_id}")
        ...
        end_date_str = result["end_date"].strftime("%Y-%m-%d")
        config_link = result.get("subscription_url") or get_text("config_link_not_available")
        
        response_text = get_text(
            "subscription_paid_from_balance_success",
            months=months,
            amount=price,
            currency="₽",
        ...
        if config_link != get_text("config_link_not_available"):
            response_text += f"\n\n🔗 {get_text('subscription_config_available')}:\n<code>{config_link}</code>"
        ...
        keyboard = get_connect_and_main_keyboard(
            ...
            i18n,
            settings,
            config_link if config_link != get_text("config_link_not_available") else None,
        ...
        await state.clear()
        await callback.answer(get_text("payment_successful_alert"), show_alert=True)
        
    except Exception as e:
        logging.error(f"Balance payment failed for user {user_id}: {e}", exc_info=True)
        await session.rollback()
        await callback.answer(get_text("error_payment_failed"), show_alert=True)
