...
# PERFORMANCE: Replaced global lock with per-user locks to prevent blocking unrelated users
# Uses dict of locks per user_id to allow concurrent payment processing for different users
_user_payment_locks: Dict[int, asyncio.Lock] = {}
_lock_cleanup_last_run = datetime.now(timezone.utc)
_LOCK_CLEANUP_INTERVAL_SECONDS = 3600  # Clean up unused locks every hour


def _get_user_payment_lock(user_id: int) -> asyncio.Lock:
    """Get or create a lock for specific user_id. Thread-safe for asyncio."""
    # PERFORMANCE: Cleanup old unused locks periodically to prevent memory leak
    global _lock_cleanup_last_run
    now = datetime.now(timezone.utc)
    if (now - _lock_cleanup_last_run).total_seconds() > _LOCK_CLEANUP_INTERVAL_SECONDS:
        # Remove locks that are not currently locked (idle locks)
        keys_to_remove = [uid for uid, lock in _user_payment_locks.items() if not lock.locked()]
        for uid in keys_to_remove:
            _user_payment_locks.pop(uid, None)
        _lock_cleanup_last_run = now
        if keys_to_remove:
            logging.debug(f"Cleaned up {len(keys_to_remove)} idle payment locks")
    
    # Get or create lock for this user
    if user_id not in _user_payment_locks:
        _user_payment_locks[user_id] = asyncio.Lock()
    return _user_payment_locks[user_id]
...
async def process_successful_payment(session: AsyncSession, bot: Bot,
    ...
    payment_type = metadata.get("payment_type", "subscription")  # "subscription" или "balance"
    ...
    # Для пополнения баланса не требуется subscription_months
    if payment_type == "balance":
        if not user_id_str or not payment_db_id_str:
            logging.error(
                f"Missing crucial metadata for balance payment: {payment_info_from_webhook.get('id')}, metadata: {metadata}"
            )
            return
    else:
        # For subscription payments (auto-renew included), payment_db_id may be absent for auto-renew.
        # In that case, we will create/ensure a payment record idempotently using provider payment id.
    ...
    try:
        ...
        if payment_type == "balance":
            logging.info(f"Processing balance deposit for user {user_id}, amount: {payment_value}")
            
            # Обновляем статус платежа
            await payment_dal.update_payment_status_by_db_id(
                session, payment_db_id, "succeeded",
            ...
            success_message = _(
                ...
                default=f"✅ Баланс успешно пополнен на {payment_value} {amount_data.get('currency', 'RUB')}!\n\n"
                       f"Текущий баланс: {new_balance} {amount_data.get('currency', 'RUB')}"
            )
            ...
            try:
                ...
                logging.error(f"Failed to send balance deposit notification to user {user_id}: {e_notify}")
            
            logging.info(f"Balance deposit successful for user {user_id}: +{payment_value}, new balance: {new_balance}")
            return
...
async def yookassa_webhook_route(request: web.Request):
    ...
    try:
        ...
        # PERFORMANCE: Extract user_id early to get per-user lock
        metadata = payment_dict_for_processing.get("metadata", {})
        user_id_str = metadata.get("user_id")
        if not user_id_str or not user_id_str.isdigit():
            logging.error(f"Missing or invalid user_id in payment webhook metadata: {metadata}")
            return web.Response(status=200, text="ok_error_no_user_id")
        
        user_id = int(user_id_str)
        user_lock = _get_user_payment_lock(user_id)
        
        # PERFORMANCE: Using per-user lock instead of global lock
