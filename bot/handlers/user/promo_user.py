...
async def process_promo_code_input(message: types.Message, state: FSMContext,
    ...
    if is_suspicious:
        ...
        if success:
            ...
            if isinstance(result, dict) and result.get("type") == "balance":
                # Промокод на баланс
                amount = result["amount"]
                response_to_user_text = _(
                    "promo_code_balance_activated",
                    amount=amount,
                    currency="₽"
                )
                reply_markup = get_back_to_main_menu_markup(current_lang, i18n)
            elif isinstance(result, datetime):
                # Промокод bonus_days (существующий код)
                new_end_date = result
                ...
                response_to_user_text = _("promo_code_applied_success")
                reply_markup = get_back_to_main_menu_markup(current_lang, i18n)
        else:
