...
class MessageQueue:
    ...
    # PERFORMANCE: Maximum queue size to prevent memory leaks during overload
    MAX_QUEUE_SIZE = 10000
    
    def __init__(self, messages_per_second: float, burst_size: int = 5):
        self.messages_per_second = messages_per_second
        self.burst_size = burst_size
        # PERFORMANCE: Using deque with maxlen for automatic overflow handling
        self.queue: deque[QueuedMessage] = deque(maxlen=self.MAX_QUEUE_SIZE)
        self.last_send_times: deque[datetime] = deque(maxlen=100)  # Keep only last 100 send times
        self.is_processing = False
        self.delay_between_messages = 1.0 / messages_per_second
        self.total_sent = 0
        self.total_failed = 0
        self.total_dropped = 0  # Track dropped messages due to queue overflow
    ...
    async def add_message(self, message: QueuedMessage) -> bool:
        ...
        PERFORMANCE: Returns False if queue is full and message was dropped.
        
        Returns:
            True if message was added, False if queue is full
        """
        # PERFORMANCE: Check if queue is at capacity before adding
        if len(self.queue) >= self.MAX_QUEUE_SIZE:
            self.total_dropped += 1
            logging.warning(
                f"Message queue full ({self.MAX_QUEUE_SIZE}), dropping message to {message.chat_id}"
            )
            return False
        
        self.queue.append(message)
        
        # PERFORMANCE: Start processing task if not already running
        if not self.is_processing:
            # Cancel old task if it exists but somehow not marked as processing
    ...
    async def _process_queue(self) -> None:
        ...
        PERFORMANCE: Properly handles task cleanup to prevent memory leaks.
        """
        ...
        try:
            while self.queue:
                ...
                try:
                    ...
                    # PERFORMANCE: Deque with maxlen handles cleanup automatically
                    # No manual cleanup needed since we set maxlen=100
            ...
            self.is_processing = False
            # PERFORMANCE: Clear task reference to allow garbage collection
...
class MessageQueueManager:
    ...
    def get_queue_stats(self) -> Dict[str, Any]:
        ...
        PERFORMANCE: Includes dropped messages count to monitor queue overflow.
        """
        return {
            ...
            "group_dropped_messages": self.group_queue.total_dropped,
            "user_dropped_messages": self.user_queue.total_dropped,
            "group_queue_capacity_pct": (len(self.group_queue.queue) / MessageQueue.MAX_QUEUE_SIZE) * 100,
            "user_queue_capacity_pct": (len(self.user_queue.queue) / MessageQueue.MAX_QUEUE_SIZE) * 100,
        }
