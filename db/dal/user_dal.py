...
async def get_user_by_id(session: AsyncSession, user_id: int) -> Optional[User]:
    # PERFORMANCE: Base query without eager loading for flexibility
    # TODO: Consider adding optional parameter for relationship loading when needed
...
async def get_banned_users(session: AsyncSession) -> List[User]:
    ...
    # PERFORMANCE: Added selectinload for subscriptions to prevent N+1 queries
    stmt = (
        select(User)
...
async def get_all_users_paginated(
    ...
    # PERFORMANCE: Added selectinload to prevent N+1 queries when displaying user subscriptions
    stmt = (
        select(User)
        .options(selectinload(User.subscriptions))
        .order_by(User.registration_date.desc())
...
async def get_all_users_with_panel_uuid(session: AsyncSession) -> List[User]:
    # PERFORMANCE: Added selectinload for subscriptions to optimize panel sync operations
    stmt = select(User).where(User.panel_user_uuid.is_not(None)).options(selectinload(User.subscriptions))
    result = await session.execute(stmt)
    return result.scalars().all()
...
async def get_user_ids_with_active_subscription(session: AsyncSession) -> List[int]:
    ...
    # PERFORMANCE: Optimized query using distinct to avoid duplicates for users with multiple subscriptions
    stmt = (
