...
async def get_payment_by_db_id(session: AsyncSession,
    ...
    # PERFORMANCE: Using selectinload to eagerly load user and promo_code relationships
...
async def get_recent_payment_logs_with_user(session: AsyncSession,
    ...
    # PERFORMANCE: Using selectinload and pagination to optimize large result sets
...
async def get_all_succeeded_payments_with_user(session: AsyncSession) -> List[Payment]:
    ...
    # PERFORMANCE: Using selectinload to prevent N+1 queries
    # TODO: Consider adding pagination for very large datasets
    stmt = (select(Payment).options(selectinload(Payment.user))
            .where(Payment.status == 'succeeded')
            .order_by(Payment.created_at.desc()))
    result = await session.execute(stmt)
...
async def get_referral_revenue(session: AsyncSession, referrer_id: int) -> float:
    ...
    # PERFORMANCE: Optimized with aggregation query - no need to load full objects
    # Uses JOIN to filter users and SUM to calculate total in database
