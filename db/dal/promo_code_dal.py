...
async def create_promo_code(session: AsyncSession,
    ...
    logging.info(
        f"Promo code '{new_promo.code}' created with ID {new_promo.promo_code_id} (Type: {new_promo.type})"
    )
    return new_promo
...
async def get_promo_codes_by_type(session: AsyncSession, promo_type: str) -> List[PromoCode]:
    stmt = select(PromoCode).where(PromoCode.type == promo_type)
    result = await session.execute(stmt)
